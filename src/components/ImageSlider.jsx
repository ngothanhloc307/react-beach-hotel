...
const ImageSlider = () => {
  ...
  const prevSilde = () => {
    ...
    const newIndex = isFirstSlide ? sildes.length - 1 : currentIndex - 1;
    setCurrentIndex(newIndex);
  };

  const nextSilde = () => {
    const isLastSlide = currentIndex === sildes.length - 1;
  ...
  return (
    <div className="max-w-[1400px] h-[580px] w-full m-auto py-16 px-4 relative group">
      <div
        ...
        style={{ backgroundImage: `url(${sildes[currentIndex].url})` }}
      ></div>
      {/* Left Arrow */}
      <div className="absolute top-[50%] translate-x-0 translate-y-[-50%] lef-5 text-2xl rounded-full p-2 group-hover:bg-black/20 text-white cursor-pointer">
        <BsChevronCompactLeft size={30} onClick={prevSilde} />
      </div>
      {/* Right Arrow */}
      <div className="absolute top-[50%] translate-x-0 translate-y-[-50%] right-5 text-2xl rounded-full p-2 group-hover:bg-black/20 text-white cursor-pointer">
        <BsChevronCompactRight size={30} onClick={nextSilde} />
      </div>
